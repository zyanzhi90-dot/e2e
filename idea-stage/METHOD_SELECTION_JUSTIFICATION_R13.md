...
Thus R13 cannot claim novelty for model/identification-to-MPC. Its delta must be the exact calibration-authorized history/finite-area normal–tangential outcome relation and its deadline-total applied action.
...
The selected Method mechanism is bounded set/tube adaptation under a three-state authority:

- SUPPORTED uses a re-entry-certified contraction;
- ADAPTING_IN_ENVELOPE immediately restores the independently calibrated full envelope before action publication while learning a new contraction;
- OUTSIDE_ENVELOPE permits only withdrawal.

This closes the abrupt-change interval left open in R12: an old contraction can never relax a hard margin. In-envelope change is handled constructively by safe full-envelope scanning when feasible and evidence-driven recontraction, rather than mere unsupported detection.

Fixed-facet set membership is the concrete update because the chosen physical parameters and bounded learned last layer yield consistency half-spaces and facet count bounds WCET. Projected EW-RLS supplies a center only. Bounded set-MHE and multiple-model banks remain credible rivals and replace it if they dominate containment, action recovery and WCET. Therefore containing authority is the Method requirement; estimator geometry is falsifiable.
...
Nominal objectives plus confidence penalties cannot authorize hard force/torque, contact, energy and actuator limits. Permanent full-envelope robust control cannot exploit new evidence. R13 separates cost and authority: a center shapes performance, while every member of the currently authorized set/tube constrains the action.
...
No Source Mechanism is formally retained in R13. VES and SPO inform Target-side derivations of action-equivalence scoping and decision-aware training, but the available provenance does not satisfy the required pre-Source discovery–terminology–search ordering, so no migrated genealogy or PASS alignment is claimed. Predict-then-calibrate contributes split-calibration discipline but no direct hard-safety guarantee. Box-FDDP may inform solver implementation only.
...
R13 is PRINCIPLE_PACKET_READY only as a reviewed set for Human Selection. It is not a FINAL_METHOD_PACKET and does not authorize Principle Test Design before the user selects.
